...
def create_app(test_config=None):
  ...
  def show_questions():
    ...
    try:
      ...
      end = min((page-1)*QUESTIONS_PER_PAGE+QUESTIONS_PER_PAGE, len(question_query))
      ...
      current_category = Category.query.filter_by(id=question_query[0].category).first().id
      return jsonify({
        "questions": questions,
  ...
  def create_search_question():
    ...
    # in the case of creating a new question 
      ...
      try:
        question_ins = Question(question=question, answer=answer, difficulty=difficulty, category=category)
  ...
  def play_quiz():
    try:
      ...
      pre_questions_id = [question_id for question_id in pre_questions] if pre_questions else []
    ...
    try:
      ...
      if pre_questions:
        for question in questions:
          if question.id not in pre_questions_id:
